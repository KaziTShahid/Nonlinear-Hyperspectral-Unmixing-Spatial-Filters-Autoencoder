# -*- coding: utf-8 -*-
"""
Created on Mon Dec 7 17:33:48 2020

@author: Kazi T Shahid and Ioannis D Schizas

TThis code is used for the script "autoencoder_main.py"
...
x = vector of abundances that will be multipled with the endmembers to obtain mixed pixels
...
class NONLIN_Layer(Layer):
    ...
    def call(self, x):
        ...
        pairs=[]
        for i in range(2,self.upto_how_many_degrees+1):    # Calculating pairs of indices to take cross-products of. "Pairs" could be of 2 for 2nd degree products, or of 3 for 3rd degree products, and so on.
        
            if (self.TAKE_REPEATING_PRODUCTS==1):
                 pairs += list(itertools.product(classes,repeat = i)) 
            else: pairs += list(itertools.combinations(classes, i))    
        
        E = tf.pad(self.linear_endmembers, ((0,len(pairs)),(0,0)), 'constant', constant_values=(0)) # Padding matrix of endmembers to contain endmember cross-products
        mask = []
